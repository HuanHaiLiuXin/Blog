...
26. Glide如何保存1个URL的多个不同尺寸的Bitmap/图片,默认保存控件指定大小的图片不算是优势,完全可以保存完整大小的,需要的时候做转换成指定大小.
27. Glide如何在列表中ItemView滑动/复用时取消不必要的请求
28. App长时间不运行,进程被回收,重新切换到App进程,如何恢复Activity数据?Activity的onSave--和onRest--属于进程被回收的情况吗?
29. onMeasure主要做什么工作,自定义View的onMeasure要注意什么?
30. RxJava如何随时取消重复/不必要的网络请求?如输入框中根据输入信息调接口,输入内容会快速变更,应该及时取消不必要的网络请求.
31. Btmap占用的内存怎么计算,和SD中占用的大小是完全无关的.默认是888,888,565这些详细区别.
32. HashMap初始容量,什么时候扩容,扩容后大小是原始的多少倍,具体怎么扩容
33. ArrayList和LinkedList存储同一份大量数据,整体耗时哪个更大(ArrayList耗时更久,因为动态数组存在初始容量,涉及扩容问题)?存储完毕后遍历哪个耗时更久(耗时应该无差别)?
```java
    private void t2(){
...
//        System.out.println("ArrayList添加耗时:"+(ea-sa));
...
//        System.out.println("LinkedList添加耗时:"+(el-sl));
...
//        }
        ...
        System.out.println("ArrayList遍历耗时:"+(ea-sa));
        sl = System.currentTimeMillis();
//        for(int i=0;i<linkedList.size();i++){
//            item = linkedList.get(i);
//        }
        Iterator<String> l = linkedList.iterator();
        while (l.hasNext()){
            item = l.next();
        }
        el = System.currentTimeMillis();
        System.out.println("LinkedList遍历耗时:"+(el-sl));
    }
    ...
    在AS里面实际测试:
    add的情况下,添加总量逐渐增大,arrayList耗时开始比LinkedList大,后来LinkedList耗时比ArrayList大.
    遍历采用iterator:
    100:都是0ms
    1000:都是0/1,看不出差别
    10000:都是1/2,看不出差别
    100000:互有高低,看不出差别
    1000000:ArrayList比LinkedList快一点,12ms:17ms,差别不大
    10000000:ArrayList比LinkedList快一倍.59ms:103ms
```
34. app打包原理
35. Integer a = Integer.valueOf("1"); Integer b = new Integer(1); Integer c = 1; 三者哪两个是==的?
```java
private void t1(){
	Integer a = Integer.valueOf("1");
	Integer b = new Integer(1);
...
```
36. Glide和其他图片加载库的区别
37. Handler涉及的Looper具体是属于谁的,存储在哪里?一个线程中会存在多个Looper吗?
38. 泛型 T 和 ? 的区别,"T,K,JK"的区别及作用?
39. RxBus
40. Retrofit及OkHttp完整流程.Retrofit下载文件.
41. 动态代理详细机制.
42. RxJava的create和just的区别,flatmap,Observer,Observable,Subscriber,Disposal的作用和联系.
43. Gradle apt
...
3. 如何避免ANR:不要再主线程做耗时操作
    ...
    - 如何避免ANR:
        ...
        4. 使用Thread或者HandlerThread时:
            - Thread:在run方法中调用Process.setThreadPriority(Process.THREAD_PRIORITY_BACKGROUND)设置优先级.**注意必须是Process.,不能是Thread.**
...
#### 24.Requestlayout,onlayout,onDraw,DrawChild区别与联系
...
- LinearLayout在子View有weight属性时,也会对所有子View调用2次onMeasure;子View没有Weight属性,所有子View仅调用1次onMeasure;
- 在不影响层级深度情况下,使用LinearLayout子View仅进行1次onMeasure,性能上优于RelativeLayout;如果使用LinearLayout会加深层级,则使用RelativeLayout.
- 提示布局性能核心就是减少层级嵌套,尽量扁平化.

#### 32.View的绘制流程:看HenCoder
